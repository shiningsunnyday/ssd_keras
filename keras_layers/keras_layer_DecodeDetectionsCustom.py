...
class DecodeDetections(Layer):
    ...
    def call(self, y_pred, mask=None):
        ...
        def filter_predictions(batch_item):
            ...
            def filter_all_classes(index):

                # From a tensor of shape (n_boxes, n_classes + 4 coordinates) extract
                # a tensor of shape (n_boxes, 1 + 4 coordinates) that contains the
                # confidnece values for just one class, determined by `index`.
                confidences = batch_item[:, 1:-4] # (n_boxes, n_classes)
                confidences_all = tf.reshape(confidences,shape=(-1,1)) # (n_boxes * n_classes, 1)
                class_id_all = tf.to_float(tf.reshape(tf.tile(tf.range(1,n_classes), tf.shape(confidences)[:1]), shape=(-1,1)))
                # coordinate_repeats = tf.fill((tf.shape(batch_item)[0],), tf.shape(confidences)[1]-1)
                coordinate_repeats = tf.convert_to_tensor([1,tf.shape(confidences)[1]])
                box_coordinates = tf.tile(batch_item[:,-4:], coordinate_repeats)
                box_coordinates = tf.reshape(box_coordinates,shape=(-1,4))

                single_class = tf.concat([class_id_all, confidences_all, box_coordinates], axis=-1)  # (nboxes * n_classes, 6)

                # Apply confidence thresholding with respect to the class defined by `index`.
                threshold_met = single_class[:,1] > self.tf_confidence_thresh
                single_class = tf.boolean_mask(tensor=single_class,
                                               mask=threshold_met) # (number of pos boxes, 6)
            ...
            filtered_single_classes = tf.map_fn(fn=lambda i: filter_all_classes(i),
                                                elems=tf.range(1,2),
                                                dtype=tf.float32,
                                                back_prop=False,
                                                swap_memory=False,
