...
class PriorBoxes(Layer):
    ...
    def build(self, input_shape):
        ...
        if K.image_dim_ordering() == 'tf':
            batch_size, feature_map_height, feature_map_width, feature_map_channels = self._keras_shape
        else:  # Not yet relevant since TensorFlow is the only supported backend right now, but it can't harm to have this in here for the future
            batch_size, feature_map_channels, feature_map_height, feature_map_width = self._keras_shape
        ...
        self.priorboxes = K.variable(boxes_tensor, dtype='float32', name='{}_priorboxes'.format(self.name))
        self.trainable_weights = [self.priorboxes]

    def call(self, x, mask=None):
        return tf.tile(self.priorboxes, tf.convert_to_tensor([tf.shape(x)[0], 1, 1, 1, 1]))
