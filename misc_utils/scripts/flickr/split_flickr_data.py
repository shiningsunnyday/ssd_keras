...
while divergence > 0.03: # forcing distribution to be very similar
    ...
    print(dist1.size,dist2.size)
    if dist1.size + dist2.size == 20 and (np.min(dist1) > 1e-5 and np.min(dist2) > 1e-5):
        divergence = kl_divergence(dist1,dist2)
        print(divergence)

X_train.to_csv('../../../../datasets/FlickrLogos_47/flickr_train_labels.csv',index=False)
X_test.to_csv('../../../../datasets/FlickrLogos_47/flickr_val_labels.csv',index=False)