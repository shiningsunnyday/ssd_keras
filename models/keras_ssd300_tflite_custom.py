...
def ssd_300(image_size,
    ...
    def l2_norm(tensor):
        axis = 3 if K.common.image_dim_ordering() == 'tf' else 1
    ...
    if mode == 'training':
        ...
        conv4_3_norm_mbox_priorbox = Lambda(anchor_boxes,
                                            output_shape=tuple(map(int, conv4_3_norm_mbox_loc.shape[1:-1])) + tuple(
                                                [n_boxes[0], 8]), arguments={'index': 0},
                                            name='conv4_3_norm_mbox_priorbox')(conv4_3_norm_mbox_loc)
        fc7_mbox_priorbox = Lambda(anchor_boxes,
                                   output_shape=tuple(map(int, fc7_mbox_loc.shape[1:-1])) + tuple([n_boxes[1], 8]),
                                   arguments={'index': 1}, name='fc7_mbox_priorbox')(fc7_mbox_loc)
        conv6_2_mbox_priorbox = Lambda(anchor_boxes,
                                       output_shape=tuple(map(int, conv6_2_mbox_loc.shape[1:-1])) + tuple(
                                           [n_boxes[2], 8]),
                                       arguments={'index': 2}, name='conv6_2_mbox_priorbox')(conv6_2_mbox_loc)
        conv7_2_mbox_priorbox = Lambda(anchor_boxes,
                                       output_shape=tuple(map(int, conv7_2_mbox_loc.shape[1:-1])) + tuple(
                                           [n_boxes[3], 8]),
                                       arguments={'index': 3}, name='conv7_2_mbox_priorbox')(conv7_2_mbox_loc)
        conv8_2_mbox_priorbox = Lambda(anchor_boxes,
                                       output_shape=tuple(map(int, conv8_2_mbox_loc.shape[1:-1])) + tuple(
                                           [n_boxes[4], 8]),
                                       arguments={'index': 4}, name='conv8_2_mbox_priorbox')(conv8_2_mbox_loc)
        conv9_2_mbox_priorbox = Lambda(anchor_boxes,
                                       output_shape=tuple(map(int, conv9_2_mbox_loc.shape[1:-1])) + tuple(
                                           [n_boxes[5], 8]),
                                       arguments={'index': 5}, name='conv9_2_mbox_priorbox')(conv9_2_mbox_loc)
        ...
        mbox_priorbox_single = tf.expand_dims(mbox_priorbox[0,:,:], axis=0)
        # mbox_priorbox_batch = tf.tile(mbox_priorbox_single, tf.convert_to_tensor([32,1,1]))
        mbox_priorbox_saved = tf.Variable(tf.zeros((1, 8732, 8)), validate_shape=True, name='mbox_priorbox_saved')
        mbox_priorbox_saved = tf.assign(mbox_priorbox_saved, mbox_priorbox_single)
        ...
        mbox_priorbox_saved_batch = Lambda(mbox_priorbox_tiled, output_shape=(8732,8), name='mbox_priorbox_tiled')(mbox_conf_softmax)
        predictions = Concatenate(axis=2, name='predictions')([mbox_conf_softmax, mbox_loc, mbox_priorbox_saved_batch])
